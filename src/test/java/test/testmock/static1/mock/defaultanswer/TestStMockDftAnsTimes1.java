...
/*
    使用PowerMockito.mockStatic方法对类进行Mock，不指定默认Answer执行真实方法
    使用Mockito.when().then...()方法对指定方法进行Stub，指定方法的真实方法不会执行
 */
@PrepareForTest({TestStaticPublicNonVoid2.class})
public class TestStMockDftAnsTimes1 extends TestStaticPublicNonVoidBase {
